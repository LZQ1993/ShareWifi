...
//    private String url = "http://192.168.120.206:8080/office/api/time?key=zhousf_key";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //设置按钮圆角样式
        aeric.sharewifi.util.SelectorFactory.newShapeSelector()
                .setStrokeWidth(2)
                .setCornerRadius(15)
        ...
        doHttpAsync(HttpInfo.Builder()
                ...
                new Callback() {
                    ...
                    public void onSuccess(HttpInfo info) throws IOException {
                        ...
                        //GSon解析
                        aeric.sharewifi.bean.TimeAndDate time = info.getRetDetail(aeric.sharewifi.bean.TimeAndDate.class);
                        aeric.sharewifi.util.LogUtil.d("MainActivity", time.getResult().toString());
                        setFromCacheTV(info);
                    }
                });
        needDeleteCache(true);
    ...
    private void deleteCache() {
        if (OkHttpUtil.getDefault().deleteCache()) {
            aeric.sharewifi.util.ToastUtil.show(this, "清理缓存成功");
        } else {
            aeric.sharewifi.util.ToastUtil.show(this, "清理缓存失败");
        }
    }
